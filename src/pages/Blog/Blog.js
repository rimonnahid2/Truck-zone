...
const Blog = () => {
  const { data: blogs = [], isLoading } = useQuery({
    queryKey: ['blogs'],
    queryFn: async () => {
      const res = await fetch(`https://track-zone-server.vercel.app/blogs`)
        ...
        <div className="advertise-section py-10">
          ...
          <div className='py-5'>
            <div className="grid grid-cols-12 gap-5">
              {blogs.map(blog =>
                <div className="col-span-6" key={blog._id}>
                  <div className="w-full overflow-hidden relative  bg-white rounded-lg shadow-lg dark:bg-gray-800">
                    <div className="px-10 py-10">
                      <h1 className="text-3xl font-bold text-[#073763] dark:text-white pb-5">{blog?.title}</h1>
