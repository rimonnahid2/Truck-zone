...
const MyProducts = () => {
  ...
  const { data: products = [], refetch, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`https://track-zone-server.vercel.app/my-products?uid=${user.uid}`, {
        headers: {
  ...
  const handleAds = (product) => {
    setLoading(true)
    fetch(`https://track-zone-server.vercel.app/product/get-ads/${product._id}`, {
      method: "PUT",
      headers: {
        authorization: `bearer ${localStorage.getItem('access-token')}`
      }
  ...
  const handleRemoveAds = (product) => {
    setLoading(true)
    fetch(`https://track-zone-server.vercel.app/product/remove-ads/${removeAds._id}`, {
      method: "PUT",
      headers: {
        authorization: `bearer ${localStorage.getItem('access-token')}`
      }
  ...
  const handleproductDelete = (product) => {
    setLoading(true)
    fetch(`https://track-zone-server.vercel.app/product/${product._id}`, {
      method: 'DELETE',
      headers: {
        authorization: `bearer ${localStorage.getItem('access-token')}`
      }
