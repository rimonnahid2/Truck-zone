...
const AddProduct = () => {
  ...
  const { data: categories = [] } = useQuery({
    queryKey: ['categories'],
    queryFn: async () => {
      const res = await fetch('https://track-zone-server.vercel.app/categories');
  ...
  const { data: brands = [] } = useQuery({
    queryKey: ['brands'],
    queryFn: async () => {
      const res = await fetch('https://track-zone-server.vercel.app/brands');
      const data = await res.json();
      return data;
    }
  })
    ...
    fetch(url, {
      ...
      .then(imgData => {
        if (imgData.success) {
          ...
          fetch('https://track-zone-server.vercel.app/product', {
            method: 'POST',
            headers: {
              ...
              <div className="bg-white px-4 py-5 sm:p-6">
                <div className="grid grid-cols-12 gap-6">
                  <div className="col-span-12 md:col-span-6">
                    <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-2">Truck Name</label>
