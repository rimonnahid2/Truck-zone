...
const Sellers = () => {
  ...
  const { data: users = [], refetch, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`https://track-zone-server.vercel.app/users?userType=${userType}`)
  ...
  const handleVerify = (user) => {
    fetch(`https://track-zone-server.vercel.app/user/verify-user/${user._id}`, {
      method: "PUT",
      headers: {
  ...
  const handleUserDelete = (user) => {
    fetch(`https://track-zone-server.vercel.app/user/${user._id}`, {
      method: 'DELETE',
      headers: {
        authorization: `bearer ${localStorage.getItem('access-token')}`
      }
  ...
  return (
    <>
      <div className="overflow-x-auto w-full">
        <h3 className="text-lg font-medium leading-6 text-gray-900 pb-5">All Sellers</h3>
        <table className="table w-full">
          ...
          <tbody>
            ...
            {users.length <= 0 &&
              <tr>
                <td colSpan="6">
                  <h2 className='text-center font-bold'>No Seller Found</h2>
                </td>
              </tr>
            }
          </tbody>
...

      {
        verify && <ConfrimAlert
          ...
          message={`If you make it, seller & seller's product will be verified.`}
